...
Esa frase es todo el problema. Un procesador de pagos que llama a otros cuatro servicios a través de HTTP antes de devolver un código 200 ha acoplado cuatro dominios de fallo a una sola solicitud de usuario. Si el email es lento, el cargo de tarjeta se siente lento. Si las herramientas de análisis están caídas, el pedido falla. Si añades un quinto cliente el trimestre que viene, tendrás que cambiar el servicio de pagos de nuevo.
...
flowchart TD
  Publisher --> Topic
  Topic --> SubA[Subscription A] --> ConsumerA[Consumidor A]
  Topic --> SubB[Subscription B] --> ConsumerB[Consumidor B]
  Topic --> SubC[Subscription C] --> ConsumerC[Consumidor C]
```
...
En un flujo de pedidos esa división se ve así:

```mermaid
flowchart TD
  OrderService[Servicio de pedidos]
  Topic["Pub/Sub Topic order-events"]
  NotifSub[Subscription de notificaciones]
  InvSub[Subscription de inventario]
  AnaSub[Subscription de analytics]
  Email["Email/SMS"]
...
Order Service publica `OrderCreated` y devuelve el resultado. No sabe quién se suscribió. Añadir un consumidor de fraude el mes que viene es una nueva subscription, no un cambio en checkout.
...
## Cuándo usar Pub/Sub — y cuándo no
...
flowchart LR
  subgraph http ["HTTP / REST"]
    HA[Servicio A] -->|petición| HB[Servicio B]
    HB -->|respuesta| HA
  end

  subgraph pubsub [Pub/Sub]
    PA[Servicio A] -->|evento| PS[Pub/Sub]
    PS --> PB[Servicio B]
    PS --> PC[Servicio C]
    PS --> PD[Servicio D]
  end
...
### At-least-once es el contrato por defecto
...
```mermaid
flowchart TD
  Msg["Message: payment.completed"] --> Process[procesa el pago]
  Process --> Save[guarda el resultado]
  Save --> AckFail[falla el ACK]
  AckFail --> Redeliver[Pub/Sub reentrega]
  Redeliver --> Again[El consumidor procesa otra vez]
```
...
```mermaid
flowchart TD
  Consume[consumir evento] --> Charge[cobrar al cliente]
  Charge --> Save[guardar resultado]
  Save --> Ack[ack]
```
...
```mermaid
flowchart TD
  Consume[consumir evento] --> Check["comprobar evento / clave de idempotencia"]
  Check --> Already{¿ya procesado?}
  Already -->|sí| AckYes[ACK]
  Already -->|no| Process[procesar evento]
  Process --> Persist[persistir resultado]
  Persist --> AckNo[ACK]
```
...
Esta es una regla de diseño, no una feature del producto. Exactly-once delivery, con cobertura posterior, reduce las _entregas_ duplicadas. No elimina la necesidad de garantizar la seguridad de los cobros, emails y actualizaciones de stock en caso de reintento.

### Exactly-once no es un default mágico
...
### Los dead-letter topics capturan veneno
...
flowchart TD
  ...
  Consumer --> Outcome{¿éxito o fallo?}
  Outcome -->|éxito| Ack[ACK]
  Outcome -->|fallo| Retry[reintento]
  Retry --> Attempts[intentos de reintento]
  Attempts --> DLT[Dead-letter Topic]
```

Después de un número aproximado de intentos de entrega — por defecto **5**, configurable de **5 a 100** — Pub/Sub puede reenviar el mensaje a un dead-letter topic. El reenvío es **best-effort**. El servicio puede mover un mensaje unos intentos antes o después, y el contador de intentos puede reiniciarse en una subscription pull idle. Diseñar para «esto aparecerá en el DLT», no para un N exacto.
...
La política de reintentos en sí es best-effort. La documentación de Google advierte contra el uso de nacks para simular delays, y contra el uso de nacks en grandes volúmenes: algunos mensajes pueden volver con poco o ningún backoff, y la entrega de toda la subscription puede ralentizarse.

Reintentos ilimitados sin dead-letter topic significa que un mensaje envenenado ocupa un slot hasta que la retención (por defecto **7 días**, de 10 minutos a 31 días) lo descarte. Eso es un descarte silencioso con un delay largo. Pon un límite a los intentos y observa qué alcanza ese límite.

## Publicar

### Batching, flow control y reintentos
...
**Publisher flow control** limita bytes y mensajes pendientes de publicación para que un pico no llene la memoria y muera con `DEADLINE_EXCEEDED`. Pon límites a la máquina en la que corres. Bloquear o dar error cuando se alcanza el límite es una elección de producto: absorbe el pico de actividad o devolverlo al emisor.
...
**Ordering** en el lado de la publicación significa endpoints regionales para que una key se quede en una región y un path de resume-publish, si un error que no admite reintentos bloquea esa key.
...
- **eventId** — único, estable, tu key de idempotencia.
...
```mermaid
flowchart TD
  Later["Quiero ejecutar este trabajo más tarde, en este endpoint"] --> Tasks[Cloud Tasks]
  Fact["Quiero publicar un hecho que varios servicios pueden consumir"] --> PubSub[Pub/Sub]
```
...
Un job de «envía este PDF de factura en 15 minutos» es Cloud Tasks. Un evento `InvoiceFinalized` que necesitan el servicio de email, el data warehouse, y el servicio de cobros es un Pub/Sub. Si solo tienes un worker conocido y necesitas que corra una vez a las 17:00, un topic añade complejidad innecesaria.
...
- **Backlog** — `subscription/num_unacked_messages_by_region` y `subscription/oldest_unacked_message_age_by_region`. Un aumento de la antigüedad es peor que un conteo creciente: unos pocos mensajes atascados harán que tu SLO expire aunque el volumen se vea bien.
- **Salud de latencia de entrega** — `subscription/delivery_latency_health_score` sobre una ventana móvil de 10 minutos. Puntúa si la subscription puede mantenerse en baja latencia, no un solo percentil que puedas citar sin la documentación.
...
**1. Reemplazar REST con Pub/Sub.** La UI necesita un id de reserva. Publicas `ReserveStock` y esperas en otro topic por `StockReserved`. Ahora tienes RPC con una política de reintentos de 7 días. Todo esto para hacer una llamada a Inventory.
...
```mermaid
flowchart TD
  When[Usa Pub/Sub cuando] --> Async[Necesitas comunicación asíncrona]
  Async --> Decouple[Necesitas desacoplamiento]
  Decouple --> FanOut[Necesitas fan-out]
  FanOut --> Tolerate[Puedes tolerar procesamiento asíncrono]
  Tolerate --> Idempotent[Diseñas para reintentos e idempotencia]
```

Si algún paso es no, detente. HTTP o Cloud Tasks es probablemente el diseño más compacto.
